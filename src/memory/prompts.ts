 ...
 * 与普通总结的关键差异——篇幅按输入规模动态约束:最终 summary 必须为待融合正文的 40%–50%,
 * 既避免过度压缩丢失关键事实,也用硬上限阻止高层节点随楼层持续膨胀。
...
注意:这是二次压缩,输入本身已是高密度总结。你必须按下方权重主动取舍和合并低优内容,严禁以“避免遗漏”为由突破篇幅上限。

【主角】{{user}}  【角色】{{char}}

【篇幅硬约束】最终 summary 的字数必须控制在下方【待融合的总结】正文总字数的 40%–50%:不得低于 40%,也绝对不得超过 50%。比例只计算 summary 字符串本身,不计算 JSON 外壳。

【待融合的总结(按时间先后,每段前的括注 (起 – 止) 是该段的真实时间范围,是你标注时间的唯一依据)】
{{content}}
...
【输出要求】
- 严格遵守 40%–50% 的篇幅硬约束;若信息放不下,按上方权重合并或舍弃低优内容,不得通过超出 50% 上限规避取舍。严格按事件发生的日期先后顺序,串联因果关系,形成一篇连贯的微型故事。
- 严禁将具体动作抽象化(❌"两人进行了交易" ✅"U用50金币换取了艾伦的地图")。
- 具体的日期、人名、地名、特定物品名必须精确保留原文。
