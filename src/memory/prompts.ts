...
import type { ItemLogEntry, MemPlan } from './types';
...
export const SUMMARY_MACROS: PromptMacro[] = [
  ...
  { token: '{{resolved_plans_block}}', desc: '近期已完成的计划/悬念(防重复记录)' },
...
{{plans_block}}
- 近期已完成的计划/悬念(已了结,只读参考;它们已结案,**禁止**再 plans.add 一遍,也不要 resolve):
...
/**
 * 渲染「近期已完成的计划/悬念」列表(不编号——它们已了结,无需像未了结项那样供 resolve 指代)。
 * 每条:[计划/悬念] 内容 (立于 X · 目标 Y),时间括注与 fmtPlans 同口径。空则「(无)」。
 */
